function buildWebs(){
            ...
            var rectangles = d3.select("body").selectAll("div")
                    .data(dataset)
                    .enter()
                    .append("div");

            rectangles.attr("fill","yellow")
                        .attr("class","bar")
                      .attr("stroke","orange")
...
var topLeftCircle = rectangles.select("#silk0")
                        .attr("fill","green");
     topLeftCircle.on("click", topLeftWeb);

var topRightCircle = rectangles.select("#silk1")
                        .attr("fill","red");
     topRightCircle.on("click", topRightWeb);
                      
var bottomRightCircle = rectangles.select("#silk3")
                        .attr("fill","orange");
     bottomRightCircle.on("click", bottomRightWeb);

var bottomLeftCircle = rectangles.select("#silk2")
                        .attr("fill","purple");
...
function drawWeb(dataset){  
    ...
    var clicked = svg.selectAll("div")	//Select all circles
                               .data(dataset);
...
let newTopRightCenterCircle = svg.select("#silk"+ ( dataset.length -3))
                                .attr("fill","red")
                                .on("click", topRightWeb);
                    
let newTopLeftCircle = svg.select("#silk"+ ( dataset.length -2))
                                .attr("fill","green")
                                .on("click", topLeftWeb);

let newBottomRightCircle = svg.select("#silk"+ ( dataset.length -1))
                                .attr("fill","orange")
...
let newTopLeftCircle = svg.select("#silk"+ ( dataset.length -3))
        .attr("fill","green")
        .on("click", topLeftWeb);

let newTopRightCenterCircle = svg.select("#silk"+ ( dataset.length -2))
                                .attr("fill","red")
                                .on("click", topRightWeb);
let newBottomRightCenterCircle = svg.select("#silk"+ ( dataset.length -1))
                                .attr("fill","purple")
...
let newBottomLeftCircle = svg.select("#silk"+ ( dataset.length -3))
                    .attr("fill","purple")
                    .on("click", bottomLeftWeb);

let newBottomTopLeftCircle = svg.select("#silk"+ ( dataset.length -2))
                    .attr("fill","red")
                    .on("click", topLeftWeb);
let newBottomRightCircle = svg.select("#silk"+ ( dataset.length -1))
                    .attr("fill","yellow")
...
let newBottomRightCenterCircle = svg.select("#silk"+ ( dataset.length -3))
                                .attr("fill","yellow")
                                .on("click", bottomRightWeb);
                    
let newBottomLeftCircle = svg.select("#silk"+ ( dataset.length -1))
                                .attr("fill","purple")
                                .on("click", bottomLeftWeb);
let newTopRightCircle = svg.select("#silk"+ ( dataset.length -2))
                                .attr("fill","red")
                                .on("click", topRightWeb);
